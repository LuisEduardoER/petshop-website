...
$(document).ready(function(){
	...
	$(".btn_msg_publish").bind("click", function() {
		...
		$.post( ctx + '/ch/user/message/publish',
				{
					rand : rand,
	...
	$(".btn_msg_reply").live("click", function() {
		...
		if (replyContent.length > 400) {
			alert("回覆内容最多可以输入400字");
			$replyContent.focus();
			return false;
		}
		$.post( ctx + '/ch/mu/user/message/reply', 
				{
            ...
            <div class="tit msg_tit_tab">
            	<c:choose>
            		<c:when test="${hasUnread==true}">
            			<a href="javascript:void(0);" class="on">未读消息</a>&nbsp;&nbsp;<a href="${ctx }/ch/mu/user/message">历史记录</a>
            		</c:when>
            		<c:otherwise>
            			历史记录
            		</c:otherwise>
            ...
            <div class="con msg_list">
                ...
                <div style="text-align: right" class="msg_more">
                	<c:if test="${!empty commonList && commonList.pageNum>1 }">
						<g:page commonList="${commonList}"  uri="${ctx}/ch/mu/user/message" target="#lay_main" pageNum="5" model="8" />  
					</c:if>
                </div>
            </div>
        </div>
